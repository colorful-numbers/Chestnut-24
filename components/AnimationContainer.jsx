...
export default function AnimationContainer({
  ...
  useEffect(() => {
    ...
    const sceneClock = createAnimationClock(root, {
      property: '--scene-time-progress',
      enterDuration: 900,
      exitDuration: 420,
    })
    const textClock = createAnimationClock(root, {
    ...
    const render = () => {
      ...
      const isSectionActive = bounds.top < window.innerHeight * 0.78
        && bounds.bottom > window.innerHeight * 0.22
      root.style.setProperty('--animation-progress', progress.toFixed(4))
      sceneClock.setTarget(isSectionActive ? 1 : 0)
      textClock.setTarget(isSectionActive ? 1 : 0)
      onProgress?.(progress)
    }
